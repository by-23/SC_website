...
export function esbuildPluginBundleDependencies({ signal = undefined, externalPackages = [], externalizeNpm = false, outDir, onMissingPackage = (pkg) => { }, }) {
            ...
            const resolveDep = async (args) => {
                ...
                if (externalPackages.some((x) => x === args.path || args.path.startsWith(x + '/'))) {
                    ...
                    if (!installedVersion) {
                        ...
                        spinner.error(`${pkg} not found: install it with \`npm i ${pkg}\` then run \`unframer\` again to generate the types`);
                    }
                    return {
                        path: args.path,
                        external: true,
