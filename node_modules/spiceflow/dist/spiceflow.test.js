...
test('* param is a path without front slash', async () => {
    ...
    {
        const res = await app.handle(new Request('http://localhost/upload/some/nested/key.txt', {
            method: 'POST',
        }));
        expect(res.status).toBe(200);
...
test('can encode superjson types', async () => {
    ...
    const res = await app.handle(new Request('http://localhost/superjson', { method: 'POST' }));
    expect(res.status).toBe(200);
...
test('handler returns url encoded data', async () => {
    ...
    const res = await new Spiceflow()
        ...
        .handle(new Request('http://localhost/form', {
        method: 'POST',
    }));
    expect(res.status).toBe(200);
...
test('onError does not fire on 404', async () => {
    ...
    const res = await app.handle(new Request('http://localhost/non-existent', { method: 'GET' }));
    expect(res.status).toBe(404);
...
test.todo('HEAD uses GET route, does not add body', async () => {
    ...
    const res = await app.handle(new Request('http://localhost/ids/xxx', { method: 'HEAD' }));
    expect(res.status).toBe(200);
    // expect(res.headers.get('Content-Length')).toBe('10')
    expect(await res.text()).toBe('');
    // Compare with GET to ensure HEAD is using GET route
    const getRes = await app.handle(new Request('http://localhost/ids/xxx', { method: 'GET' }));
...
test('validate body works, request fails', async () => {
    const res = await new Spiceflow()
        .post('/post', async ({ request, redirect }) => {
        // console.log({request})
        let body = await request.json();
        expect(body).toEqual({ name: 'John' });
    }, {
    ...
    expect(res.status).toBe(422);
    expect(await res.text()).toMatchInlineSnapshot(`"{"code":"VALIDATION","status":422,"message":"requiredField: Required"}"`);
});
test('run use', async () => {
    const res = await new Spiceflow()
        .use(({ request }) => {
...
test('basPath works with use', async () => {
    let app = new Spiceflow({ basePath: '/one' }).use(new Spiceflow({})
        .get('/two', () => 'hi')
        .use(new Spiceflow({ basePath: '/three' }).get('/four', () => 'hi')));
    {
        const res = await app.handle(new Request('http://localhost/one/two', { method: 'GET' }));
        expect(res.status).toBe(200);
        expect(await res.json()).toEqual('hi');
    }
    {
        const res = await app.handle(new Request('http://localhost/one/three/four', { method: 'GET' }));
        expect(res.status).toBe(200);
        expect(await res.json()).toEqual('hi');
    }
});
test('getRouteAndParents', async () => {
    let app = new Spiceflow({ basePath: '/one' })
        .get('/ids/:id', () => 'hi')
        .use(new Spiceflow({ basePath: '/two' }).use(new Spiceflow({ basePath: '/three' }).get('/four', () => 'hi')));
    let routers = bfs(app);
...
test('getAppsInScope include all parent apps', async () => {
    let app = new Spiceflow({ basePath: '/one' })
        .get('/ids/:id', () => 'hi')
        .use(new Spiceflow({ basePath: '/two' }).use(new Spiceflow({ basePath: '/three' }).use(new Spiceflow({ basePath: '/four' })
        .get('/five', () => 'hi')
        .use(({ request }) => { }))));
    let routers = bfs(app);
...
test('getAppsInScope include all parent apps and non scoped apps', async () => {
    let app = new Spiceflow({ basePath: '/one' })
        .get('/ids/:id', () => 'hi')
        .use(new Spiceflow({ basePath: '/two' }).use(new Spiceflow({ basePath: '/three' }).use(new Spiceflow({ basePath: '/four', scoped: false })
        .get('/five', () => 'hi')
        .use(({ request }) => { }))));
    let routers = bfs(app);
    let secondLast = routers[routers.length - 2];
...
test('use with 2 basPath works', async () => {
    ...
    const app = await new Spiceflow()
        ...
        .use(new Spiceflow({ basePath: '/one' })
        .use(({ request }) => {
        oneOnReq = true;
        onReqCalled.push('one');
    })
        .get('/ids/:id', ({ params }) => params.id))
        .use(new Spiceflow({ basePath: '/two' })
        .use((c) => {
        twoOnReq = true;
...
test('use with nested basPath works', async () => {
    const app = await new Spiceflow({ basePath: '/zero' })
        .use(new Spiceflow({ basePath: '/one' }).get('/ids/:id', ({ params }) => params.id))
        .use(new Spiceflow({ basePath: '/two' }).use(new Spiceflow({ basePath: '/nested' }).get('/ids/:id', ({ params }) => params.id)));
    {
...
test('errors inside basPath works', async () => {
    ...
    const app = await new Spiceflow({ basePath: '/zero' })
        ...
        .use(new Spiceflow({ basePath: '/nested' })
        .onError(({ error }) => {
...
describe('Trailing slashes and base paths', () => {
    test('App without trailing slash, request with trailing slash', async () => {
        const app = new Spiceflow({ basePath: '/api' }).get('/users', () => new Response('Users list'));
    ...
    test('Nested routes with and without trailing slashes', async () => {
        const app = new Spiceflow({ basePath: '/api' }).use(new Spiceflow()
            .get('/products', () => new Response('Products list'))
            .get('/categories/', () => new Response('Categories list')));
        const resProducts = await app.handle(new Request('http://localhost/api/products/'));
        expect(resProducts.status).toBe(200);
        expect(await resProducts.text()).toBe('Products list');
        const resCategories = await app.handle(new Request('http://localhost/api/categories'));
