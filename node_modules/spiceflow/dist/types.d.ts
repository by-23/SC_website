...
export type UnwrapSchema<Schema extends TypeSchema | string | undefined, Definitions extends Record<string, unknown> = {}> = Schema extends undefined ? unknown : Schema extends StandardSchemaV1 ? StandardSchemaV1.InferOutput<Schema> : Schema extends ZodTypeAny ? z.infer<Schema> : Schema extends string ? Definitions extends Record<Schema, infer NamedSchema> ? NamedSchema : Definitions : unknown;
export type GetRequestSchema<Schema extends InputSchema<any>> = 'request' extends keyof Schema ? Schema['request'] : 'body' extends keyof Schema ? Schema['body'] : undefined;
...
export type ContentType = MaybeArray<(string & {}) | 'text/plain' | 'application/json' | 'multipart/form-data' | 'application/x-www-form-urlencoded'>;
...
export interface MergeSchema<in out A extends RouteSchema, in out B extends RouteSchema> {
    request: undefined extends GetRequestSchema<A> ? GetRequestSchema<B> : GetRequestSchema<A>;
...
export type InlineHandler<This, Route extends RouteSchema = {}, Singleton extends SingletonBase = {
    state: {};
}, Path extends string = '', MacroContext = {}> = (this: This, context: MacroContext extends Record<string | number | symbol, unknown> ? Prettify<MacroContext & Context<Route, Singleton, Path>> : Context<Route, Singleton, Path>) => Response | MaybePromiseIterable<{} extends Route['response'] ? unknown : (Route['response'] extends {
    200: any;
...
export type BodyHandler<in out Route extends RouteSchema = {}, in out Singleton extends SingletonBase = {
    ...
    contentType: string;
} & Context<Route, Singleton, Path>>, contentType: string) => MaybePromise<any>;
export type MiddlewareHandler<in out Route extends RouteSchema = {}, in out Singleton extends SingletonBase = any> = (context: MiddlewareContext<Singleton>, next: () => Promise<Response>) => MaybePromise<Route['response'] | void>;
...
} & Partial<Singleton['state']>> | Prettify<{
    request: Request;
    code: 'VALIDATION';
    error: Readonly<ValidationError>;
} & Singleton['state']> | Prettify<{
    [K in keyof T]: {
        request: Request;
        code: K;
        error: Readonly<T[K]>;
    };
}[keyof T] & Partial<Singleton['state']>>)) => any | Promise<any>;
...
type _CreateClient<Path extends string, Property extends Record<string, unknown> = {}> = Path extends `${infer Start}/${infer Rest}` ? {
    [x in Start]: _CreateClient<Rest, Property>;
} : {
    [x in Path]: Property;
};
export type CreateClient<Path extends string, Property extends Record<string, unknown> = {}> = Path extends `/${infer Rest}` ? _CreateClient<Rest, Property> : Path extends '' ? _CreateClient<'index', Property> : _CreateClient<Path, Property>;
export type ComposeSpiceflowResponse<Response, Handle> = Handle extends (...a: any[]) => infer A ? _ComposeSpiceflowResponse<Response, Awaited<A>> : _ComposeSpiceflowResponse<Response, Awaited<Handle>>;
type _ComposeSpiceflowResponse<Response, Handle> = Prettify<{} extends Response ? {
    200: Exclude<Handle, {
        [SPICEFLOW_RESPONSE]: any;
    }>;
} & {
...
export type MergeSpiceflowInstances<Instances extends AnySpiceflow[] = [], Prefix extends string = '', Scoped extends boolean = false, Singleton extends SingletonBase = {
    state: {};
...
export type GetPathsFromRoutes<Routes extends Record<string, unknown>> = Routes extends Record<infer K, any> ? (K extends string ? K : never) : never;
export type ExtractParamsFromPath<Path extends string> = Path extends `${string}:${infer Param}/${infer Rest}` ? {
    [K in Param]: string;
} & ExtractParamsFromPath<`/${Rest}`> : Path extends `${string}:${infer Param}` ? {
    [K in Param]: string;
